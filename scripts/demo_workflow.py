...
class MCPClient:
    ...
    def call_service(self, service: str, endpoint: str, data: Dict[Any, Any] = None) -> Dict[Any, Any]:
...
async def run_demo_workflow():
    ...
    while attempt < max_attempts:
        status_result = client.call_service("workflow", f"/get_workflow_status", {
        ...
        print(f"📊 Status: {current_status}" + (f" | Current Step: {current_step}" if current_step else ""))
    ...
    final_status = client.call_service("workflow", f"/get_workflow_status", {
        "workflow_id": workflow_id
    })
    ...
    if "error" not in final_status:
        ...
        for i, step in enumerate(final_status.get('steps', []), 1):
            ...
            print(f"  {status_emoji} Step {i}: {step.get('name')} ({step.get('status')})")
    
    # Step 4: Show metrics
    print(f"\n📊 Step 4: System Metrics")
    
    metrics_result = client.call_service("workflow", "/get_workflow_metrics", {
        "timeframe_days": 1
    })
    ...
    if "error" not in metrics_result:
        ...
        print(f"Average Duration: {metrics_result.get('average_duration_seconds', 0):.1f}s")
    
    print(f"\n🎉 Demo workflow completed!")
    print(f"Check Grafana at http://localhost:3000 for detailed metrics")
