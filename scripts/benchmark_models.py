...
class ModelBenchmark:
    ...
    async def benchmark_model(
        self, model: str, num_iterations: int = 3
    ) -> BenchmarkSuite:
        ...
        logger.info(
            f"Completed benchmarking {model}: {suite.successful_tests}/{suite.total_tests} successful"
        )
        return suite

    async def run_single_test(
        self, model: str, test_name: str, prompt: str
    ) -> BenchmarkResult:
    ...
    async def benchmark_rag_integration(self, model: str) -> Dict[str, Any]:
        ...
        if successful_results:
            avg_context_time = statistics.mean(
                [r["context_retrieval_time"] for r in successful_results]
            )
            ...
            avg_context_length = statistics.mean(
                [r["context_length"] for r in successful_results]
            )
            ...
            rag_metrics = {
                ...
                "context_utilization_rate": len(
                    [r for r in successful_results if r["context_length"] > 0]
                )
    ...
    async def run_comprehensive_benchmark(self) -> Dict[str, Any]:
        ...
        for model in available_models:
            try:
                ...
                benchmark_results[model] = {"error": str(e), "timestamp": time.time()}

        return {
            "benchmark_summary": {
                "total_models_tested": len(benchmark_results),
                "successful_models": len(
                    [r for r in benchmark_results.values() if "error" not in r]
                ),
                "test_timestamp": time.time(),
                "test_scenarios_count": sum(
                    len(prompts) for prompts in self.test_scenarios.values()
                ),
            },
    ...
    def print_summary(self, results: Dict[str, Any]):
        ...
        for model, data in results.get("model_results", {}).items():
            ...
            print(f"\\n{model}:")
            print(
                f"  Standard Tests: {std_bench.get('successful_tests', 0)}/{std_bench.get('total_tests', 0)} passed"
            )
            ...
            print(
                f"  RAG Integration: {rag_bench.get('successful_queries', 0)}/{rag_bench.get('total_queries', 0)} passed"
            )
...
async def main():
    ...
    print(
        "This will test response time, quality, and RAG integration across all available models."
    )
