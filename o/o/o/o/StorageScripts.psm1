...
{
    [CmdletBinding( DefaultParameterSetName = "ByFriendlyName", ConfirmImpact="High" )]
    Param
    (
        #### -------------------- Parameter sets -------------------------------------####
    ...
    Process
    {
        ...
        try
        {
            ...
            switch ($psCmdlet.ParameterSetName)
            {
                ...
                "ByFriendlyName" { $Instances = Get-PhysicalDisk -FriendlyName $FriendlyName -CimSession $CimSession -ErrorAction Stop; break; }
            }

            if (-not $AsJob)
            {
                Write-Progress -Activity "Convert-PhysicalDisk" -PercentComplete 0 -CurrentOperation "Validating parameters" -Status "0/2"
            }
            ...
            {
                $arguments.Add("Format", [System.UInt16]$Format)
            }

            if ($PSBoundParameters.ContainsKey("StoragePoolFriendlyName"))
            {
                $arguments.Add("StoragePoolFriendlyName", $StoragePoolFriendlyName)
            }

            if ($PSBoundParameters.ContainsKey("StoragePoolVersion"))
            {
                $arguments.Add("StoragePoolVersion", [System.UInt16]$StoragePoolVersion)
            }

            if ($PSBoundParameters.ContainsKey("StoragePoolMetadataLength"))
            {
                $arguments.Add("StoragePoolMetadataLength", [System.UInt64]$StoragePoolMetadataLength)
            }

            if ($PSBoundParameters.ContainsKey("StoragePoolMinimumAllocationSize"))
            {
                $arguments.Add("StoragePoolMinimumAllocationSize", [System.UInt64]$StoragePoolMinimumAllocationSize)
            }

            if ($PSBoundParameters.ContainsKey("VirtualDiskFriendlyName"))
            {
                $arguments.Add("VirtualDiskFriendlyName", $VirtualDiskFriendlyName)
            }

            if ($PSBoundParameters.ContainsKey("VirtualDiskProvisioningType"))
            {
                $arguments.Add("VirtualDiskProvisioningType", [System.UInt16]$VirtualDiskProvisioningType)
            }

            if ($PSBoundParameters.ContainsKey("VirtualDiskAllocationUnitSize"))
            {
                $arguments.Add("VirtualDiskAllocationUnitSize", [System.UInt64]$VirtualDiskAllocationUnitSize)
            }

            if ($PSBoundParameters.ContainsKey("VirtualDiskResiliencySettingName"))
            {
                $arguments.Add("VirtualDiskResiliencySettingName", [System.UInt16]$VirtualDiskResiliencySettingName)
            }
            ...
            $methodblock = {
                ...
                if ($instances.Length -gt 1)
                {
                    $errorObject = CreateErrorRecord -ErrorId "NotSupported" `
                                                     -ErrorMessage "Conversion is not supported for more than 1 drive" `
                                                     -ErrorCategory ([System.Management.Automation.ErrorCategory]::NotEnabled) `
                                                     -Exception $null `
                                                     -TargetObject $null
                ...
                if (-not $asjob)
                {
                    Write-Progress -Activity "Convert-PhysicalDisk" -PercentComplete 30 -CurrentOperation "Converting PhysicalDisk" -Status "1/2"
                }

                ForEach ($instance in $instances)
                {
                    $Disk = (Get-Disk -Number $instance.DeviceId -ErrorAction SilentlyContinue)

                    if ($Disk -eq $null)
                    {
                        $errorObject = CreateErrorRecord -ErrorId "ObjectNotFound" `
                                                         -ErrorMessage "The disk associated with the physical disk (DeviceId: '$($instance.DeviceId)' and UniqueId: '$($instance.UniqueId)') could not be found." `
                                                         -ErrorCategory ([System.Management.Automation.ErrorCategory]::ObjectNotFound) `
                                                         -Exception $null `
                                                         -TargetObject $null
                    ...
                    if ($Disk.IsSystem -and $Disk.IsBoot)
                    {
                        $IsBootDevice = $True
                    }

                    if ($Disk.PartitionStyle -eq "MBR" -and
                        $IsBootDevice)
                    {
                        $errorObject = CreateErrorRecord -ErrorId "NotSupported" `
                                                         -ErrorMessage "Conversion is not supported for MBR boot drive" `
                                                         -ErrorCategory ([System.Management.Automation.ErrorCategory]::NotEnabled) `
                                                         -Exception $null `
                                                         -TargetObject $null
                    ...
                    try
                    {
                        ...
                        $output = Invoke-CimMethod -CimSession $session -InputObject $instance -MethodName "Convert" -Arguments $arguments -ErrorAction Stop

                        #### ------------------ If it is a boot device copy the boot file ------------####

                        if ($IsBootDevice)
                        {
                            # Find the VirtualDisk with user provided FriendlyName

                            $SpaceDisk = (Get-Disk -FriendlyName $VirtualDiskFriendlyName)

                            $Partition = New-Partition -DiskNumber $SpaceDisk.Number -GptType "{c12a7328-f81f-11d2-ba4b-00a0c93ec93b}" `
                            ...
                            # Unmount the vol, so the system partition
                        ...
                        $storageObject = $output.CreatedStorageObject
                    }
                    catch
                    {
                        $progressPreference = "Continue"
                    ...
                    finally
                    {
                        if (-not $asjob)
                        {
                            Write-Progress -Activity "Convert-PhysicalDisk" -Completed -Status "2/2"
                        }
                    }
                }
            ...
            if ($asjob)
            {
                ...
                Start-Job -Name ConvertPhysicalDisk -ScriptBlock $methodblock -ArgumentList @($CimSession, $p, $Instances, $arguments)
            }
            else
            {
