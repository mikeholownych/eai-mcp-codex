...
export class StaffApiService {
  ...
  static async assignTicket(
    ...
    const response = await api.patch(
      `${STAFF_API_BASE}/tickets/${id}/assign?assign_to=${assignTo}`,
    );
    return response.data;
  }
  ...
  static async updateTicketStatus(
    ...
    const response = await api.patch(
      `${STAFF_API_BASE}/tickets/${id}/status?new_status=${status}`,
    );
    return response.data;
  }
...
export function handleStaffApiError(error: unknown): string {
  ...
  if (typedError.response?.status && typedError.response.status >= 500) {
