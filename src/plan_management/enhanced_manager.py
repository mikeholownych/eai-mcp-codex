...
class EnhancedPlanManager(PlanManager):
    ...
    async def create_enhanced_plan(self, request: PlanRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.enable_predictions:
                try:
                    prediction_result = await self.prediction_engine.predict_workflow_duration(plan)
                    ...
                    if not plan.estimated_hours:
                        self.update_plan(plan.id, {'estimated_hours': prediction_result.estimated_duration})
                        plan.estimated_hours = prediction_result.estimated_duration
                    
                    logger.info(f"Generated predictions for plan {plan.id}: {prediction_result.estimated_duration:.1f}h")
            ...
            if self.enable_security_scanning and hasattr(request, 'code_context'):
                try:
                    security_report = await self.security_analyzer.analyze_code_security(
                    ...
                    enhancements['security_assessment'] = {
                        ...
                        'compliance_status': len([
                            c for c in security_report.compliance_status 
                            if c.status == 'pass'
                        ]),
                    ...
                    logger.info(f"Security assessment completed for plan {plan.id}: score {security_report.security_score:.1f}")
    ...
    def _generate_risk_analysis(self, plan: Plan, predictions: Optional[Dict], 
                               security: Optional[Dict]) -> Dict[str, Any]:
        """Generate comprehensive risk analysis."""
        risk_factors = []
        overall_risk = 'low'
    ...
    def _generate_recommendations(self, plan: Plan, enhancements: Dict) -> List[Dict[str, Any]]:
    ...
    async def _store_enhancement_metadata(self, plan_id: str, enhancements: Dict):
        ...
        try:
            metadata = {
                ...
                'enhancement_summary': {
                    'predictions_generated': enhancements.get('predictions') is not None,
                    'security_assessed': enhancements.get('security_assessment') is not None,
                    'risk_analyzed': enhancements.get('risk_analysis') is not None,
                    'recommendations_count': len(enhancements.get('recommendations', []))
                }
            }
    ...
    async def get_plan_analytics(self, plan_id: str) -> Dict[str, Any]:
        ...
        if self.enable_predictions:
            try:
                prediction = await self.prediction_engine.predict_workflow_duration(plan)
