...
class DependencyGraphAnalyzer:
    ...
    def calculate_slack_times(self, critical_path: List[str]) -> Dict[str, float]:
        ...
        for task_id in self.graph.nodes():
            if task_id not in critical_path:
                ...
                slack_times[task_id] = self._calculate_task_slack(task_id, critical_path)
    ...
    def _calculate_task_slack(self, task_id: str, critical_path: List[str]) -> float:
        ...
        paths_through_task = list(nx.all_simple_paths(self.graph, 
...
class ResourceOptimizer:
    ...
    def optimize_resource_allocation(self, tasks: List[ScheduledTask], 
                                   strategy: SchedulingStrategy) -> Dict[str, List[Resource]]:
    ...
    def _sort_tasks_for_allocation(self, tasks: List[ScheduledTask], 
                                  strategy: SchedulingStrategy) -> List[ScheduledTask]:
    ...
    def _allocate_resources_for_task(self, task: ScheduledTask, 
                                   strategy: SchedulingStrategy) -> List[Resource]:
        ...
        for resource_type, required_amount in task.resource_requirements.items():
            suitable_resources = [
                ...
                r.capacity - r.current_usage >= required_amount
            ]
            
            if not suitable_resources:
                logger.warning(f"No suitable {resource_type} resources for task {task.task_id}")
                continue
    ...
    def calculate_resource_utilization(self, allocation: Dict[str, List[Resource]]) -> Dict[str, float]:
        """Calculate resource utilization metrics."""
        utilization = {}
        
        for resource in self.available_resources:
            utilization[resource.resource_id] = resource.current_usage / resource.capacity
        
        return utilization
    
    def estimate_cost(self, allocation: Dict[str, List[Resource]], 
...
class GeneticAlgorithmOptimizer:
    ...
    def optimize_schedule(self, tasks: List[ScheduledTask], 
                         constraints: Dict[str, Any],
                         objective: SchedulingStrategy) -> List[str]:
        """Optimize task schedule using genetic algorithm."""
        logger.info(f"Starting genetic algorithm optimization with {self.population_size} individuals")
        ...
        for generation in range(self.generations):
            ...
            for individual in population:
                fitness = self._evaluate_fitness(individual, tasks, constraints, objective)
    ...
    def _evaluate_fitness(self, individual: List[str], tasks: List[ScheduledTask],
                         constraints: Dict[str, Any], objective: SchedulingStrategy) -> float:
    ...
    def _calculate_dependency_penalty(self, individual: List[str], 
                                    tasks: List[ScheduledTask]) -> float:
    ...
    def _estimate_schedule_duration(self, individual: List[str], 
                                   tasks: List[ScheduledTask]) -> float:
    ...
    def _calculate_parallelism_score(self, individual: List[str], 
                                   tasks: List[ScheduledTask]) -> float:
    ...
    def _evolve_population(self, population: List[List[str]], 
...
class IntelligentScheduler:
    ...
    def __init__(self, available_resources: List[Resource] = None):
        ...
        if not available_resources:
            self.resource_optimizer.available_resources = self._create_default_resources()
    ...
    async def optimize_task_scheduling(self, task_list: List[Task], 
                                     constraints: Dict[str, Any] = None,
                                     objective: SchedulingStrategy = SchedulingStrategy.MINIMIZE_DURATION) -> SchedulingPlan:
        ...
        dependency_graph = self.dependency_analyzer.build_dependency_graph(scheduled_tasks)
        ...
        # Calculate timing
        total_duration = await self._calculate_schedule_timing(scheduled_tasks, optimized_order)
        
        # Calculate metrics
        parallel_efficiency = self._calculate_parallel_efficiency(scheduled_tasks, total_duration)
        resource_utilization = self.resource_optimizer.calculate_resource_utilization(resource_allocation)
        estimated_cost = self.resource_optimizer.estimate_cost(resource_allocation, total_duration)
        ...
        # Generate warnings
        warnings = self._generate_scheduling_warnings(scheduled_tasks, resource_utilization)
        ...
        logger.info(f"Scheduling completed: {total_duration:.1f}h duration, {optimization_score:.2f} score")
    ...
    def _estimate_resource_requirements(self, task: Task) -> Dict[str, float]:
        ...
        elif any(keyword in description for keyword in ['data', 'analysis', 'processing']):
    ...
    def _calculate_task_priority(self, task: Task) -> int:
        ...
        if any(keyword in description for keyword in ['critical', 'urgent', 'important']):
    ...
    async def _calculate_criticality_scores(self, tasks: List[ScheduledTask], critical_path: List[str]):
    ...
    async def _calculate_schedule_timing(self, tasks: List[ScheduledTask], 
                                       optimized_order: List[str]) -> float:
        ...
        for task_id in optimized_order:
                ...
                if dep.predecessor in task_dict:
                    ...
                    if pred_task.scheduled_finish:
                        earliest_start = max(earliest_start, 
                                           pred_task.scheduled_finish + dep.lag_time)
            ...
            task.scheduled_finish = datetime.utcnow() + timedelta(hours=earliest_start + task.estimated_duration)
            
            # Update resource availability
            for resource in task.assigned_resources:
                resource_availability[resource.resource_id] = earliest_start + task.estimated_duration
        
        # Total duration is the latest finish time
        if tasks:
            finish_times = [
                (task.scheduled_finish - datetime.utcnow()).total_seconds() / 3600
                for task in tasks if task.scheduled_finish
            ]
            return max(finish_times) if finish_times else 0.0
        
        return 0.0
    
    def _calculate_parallel_efficiency(self, tasks: List[ScheduledTask], total_duration: float) -> float:
    ...
    def _calculate_optimization_score(self, total_duration: float, parallel_efficiency: float,
                                    resource_utilization: Dict[str, float], 
                                    objective: SchedulingStrategy) -> float:
    ...
    def _generate_scheduling_warnings(self, tasks: List[ScheduledTask], 
                                    resource_utilization: Dict[str, float]) -> List[str]:
        ...
        for resource_id, utilization in resource_utilization.items():
            if utilization > 0.9:
                warnings.append(f"Resource {resource_id} is over-utilized ({utilization:.1%})")
        
        # Check for under-utilized resources
        underutilized = [r_id for r_id, util in resource_utilization.items() if util < 0.3]
        if len(underutilized) > len(resource_utilization) / 2:
            warnings.append("Many resources are under-utilized - consider reducing resource allocation")
        ...
        if len(critical_tasks) > len(tasks) * 0.7:
            warnings.append("Many tasks are on or near critical path - schedule has limited flexibility")
        
        # Check for resource conflicts
        high_resource_tasks = [
            task for task in tasks 
            if sum(task.resource_requirements.values()) > 10
        ]
        if len(high_resource_tasks) > 3:
            warnings.append("Multiple high-resource tasks may cause scheduling conflicts")
