...
class AgentPoolManager:
    ...
    async def create_agent_instance(
        self, agent_type: AgentType, agent_name: str
    ) -> AgentInstance:
    ...
    async def submit_task(self, task: TaskRequest) -> bool:
        ...
        try:
            ...
            priority_queue = self.task_queue.get(
                task.priority.value, self.task_queue["medium"]
            )
    ...
    async def get_available_agent(
        self, agent_type: AgentType, required_capabilities: List[str] = None
        ...
        candidates = [
            ...
            if (
                ...
                and agent.state == AgentState.IDLE
                and agent.workload < agent.max_concurrent_tasks
            )
        ]
    ...
    async def assign_task_to_agent(
        self, task: TaskRequest, agent: AgentInstance
    ) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to assign task {task.task_id} to agent {agent.instance_id}: {e}"
            )
            return False
        ...
        try:
            ...
            if "avg_execution_time" not in agent.performance_metrics:
                ...
                agent.performance_metrics["avg_execution_time"] = (
                    current_avg + task_result.execution_time
                ) / 2

            agent.performance_metrics["total_tasks"] = (
                agent.performance_metrics.get("total_tasks", 0) + 1
            )
            if task_result.status == "completed":
                agent.performance_metrics["success_rate"] = (
                    agent.performance_metrics.get("success_rate", 0) + 1
                )
            ...
            self.redis.setex(
                result_key, 86400, task_result.model_dump_json()
            )  # 24 hours TTL
            ...
            logger.info(
                f"Task {task_result.task_id} completed by agent {task_result.agent_instance_id}"
            )
    ...
    async def get_workload_distribution(self) -> List[WorkloadDistribution]:
        ...
        for agent_type in AgentType:
            ...
            active_instances = len(
                [a for a in agents_of_type if a.state != AgentState.MAINTENANCE]
            )
            idle_instances = len(
                [a for a in agents_of_type if a.state == AgentState.IDLE]
            )
            working_instances = len(
                [a for a in agents_of_type if a.state == AgentState.WORKING]
            )
    ...
    async def scale_agent_pool(self) -> Dict[str, int]:
        ...
        for dist in distributions:
            ...
            if (
                dist.utilization_rate > self.config.scale_up_threshold
                and dist.total_instances
                < self.config.max_agents_per_type[dist.agent_type]
            ):
            ...
            # Scale down if utilization is low
                ...
                dist.utilization_rate < self.config.scale_down_threshold
                and dist.idle_instances > self.config.min_idle_agents
            ):
        ...
        for priority in ["urgent", "high", "medium", "low"]:
            ...
            for task in tasks_to_process:
                agent = await self.get_available_agent(
                    task.required_agent_type, task.required_capabilities
                )
