...
class KubernetesCluster:
    ...
    async def deploy(self, deployment_plan: DeploymentPlan) -> DeploymentResult:
        ...
        try:
            ...
            health_status = await self._perform_health_checks(
                deployment_plan.health_checks, endpoints
            )
    ...
    def _create_deployment_manifest(self, deployment_plan: DeploymentPlan) -> str:
        ...
        manifest = {
            ...
            "spec": {
                ...
                "selector": {
                    "matchLabels": {"app": f"mcp-{deployment_plan.workflow_id}"}
                },
                "template": {
                    "metadata": {
                        "labels": {
                    ...
                    "spec": {
                        "containers": [
                            {
                                "name": "mcp-container",
                                "image": deployment_plan.configuration.get(
                                    "image", "mcp-default:latest"
                                ),
    ...
    def _create_service_manifest(self, deployment_plan: DeploymentPlan) -> str:
        ...
        manifest = {
            ...
            "metadata": {
                ...
                "namespace": self.namespace,
                "labels": {"app": f"mcp-{deployment_plan.workflow_id}"},
            },
            "spec": {
                "selector": {"app": f"mcp-{deployment_plan.workflow_id}"},
    ...
    async def _apply_manifest(
        self, manifest_type: str, manifest_content: str
    ...
    async def _wait_for_ready(self, deployment_id: str, timeout: int = 300):
        ...
        while (datetime.utcnow() - start_time).total_seconds() < timeout:
            try:
                cmd = [
                    ...
                    "rollout",
                    "status",
                    f"deployment/mcp-{deployment_id}",
                    "-n",
                    self.namespace,
                ]
                if self.config_path:
        ...
        raise Exception(
            f"Deployment {deployment_id} did not become ready within {timeout} seconds"
        )
    ...
    async def _perform_health_checks(
        self, health_checks: List[Dict[str, Any]], endpoints: List[str]
    ) -> str:
    ...
    async def deploy(self, deployment_plan: DeploymentPlan) -> DeploymentResult:
        ...
        try:
            ...
            return DeploymentResult(
                deployment_id=deployment_plan.deployment_id,
                status=(
                    DeploymentStatus.DEPLOYED if resource else DeploymentStatus.FAILED
                ),
                deployed_resources=deployed_resources,
                endpoints=self._get_aws_endpoints(resource) if resource else [],
                logs=logs,
                metrics={"deployment_duration": duration, "aws_region": self.region},
                duration=duration,
            )
...
class AzureEnvironment:
    ...
    def _get_azure_endpoints(self, resource: CloudResource) -> List[str]:
        ...
        if resource.resource_type == "container_instance":
            ...
            return [
                f"https://{resource.resource_id}.{self.location}.cloudapp.azure.com"
            ]
        return []
...
class CloudOrchestrator:
    ...
    async def deploy_to_environment(
        ...
        try:
            ...
            deployment_plan = await self._generate_deployment_plan(
                workflow_id, target_env, configuration
            )
            ...
            logger.info(
                f"Deployment {result.deployment_id} completed with status: {result.status}"
            )
    ...
    async def _generate_deployment_plan(
        ...
        if target_env == EnvironmentType.PRODUCTION:
            ...
            default_config["resources"] = {"cpu": "500m", "memory": "1Gi"}
        elif target_env == EnvironmentType.STAGING:
            default_config["replicas"] = 2
            default_config["resources"] = {"cpu": "250m", "memory": "512Mi"}
        else:
            default_config["replicas"] = 1
            default_config["resources"] = {"cpu": "100m", "memory": "256Mi"}

        # Generate health checks
        health_checks = [
            {"type": "http", "path": "/health", "expected_status": 200, "timeout": 10},
            {"type": "http", "path": "/ready", "expected_status": 200, "timeout": 5},
        ]
    ...
    async def list_deployments(self, workflow_id: str = None) -> List[DeploymentResult]:
        ...
        if workflow_id:
            return [
                d for d in self.deployment_history if workflow_id in d.deployment_id
            ]
