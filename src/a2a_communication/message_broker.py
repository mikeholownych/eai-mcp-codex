...
from .models import A2AMessage, AgentRegistration, AgentStatus, MessageType
...
class A2AMessageBroker:
    ...
    def send_message(self, message: A2AMessage) -> bool:
        ...
        try:
            ...
            if message.recipient_agent_id:
                ...
                self.redis.expire(queue_key, self.message_ttl)
            else:
                # Broadcast message
                self.redis.publish("agent:broadcast", message_data)
            
            # Store message history
            history_key = f"conversation:{message.conversation_id}:messages"
            self.redis.lpush(history_key, message_data)
            self.redis.expire(history_key, 86400)  # 24 hours
            
            logger.info(f"Message {message.id} sent to {message.recipient_agent_id or 'broadcast'}")
            return True
    ...
    def get_messages(self, agent_id: str, limit: int = 10) -> List[A2AMessage]:
        ...
        try:
            ...
            raw_messages = self.redis.lrange(queue_key, 0, limit - 1)
            ...
            if raw_messages:
                self.redis.ltrim(queue_key, len(raw_messages), -1)
    ...
    def register_agent(self, registration: AgentRegistration) -> bool:
        ...
        try:
            ...
            self.redis.setex(agent_key, 300, agent_data)  # 5 minutes TTL
            
            # Add to agent list by type
            type_key = f"agents:type:{registration.agent_type}"
            self.redis.sadd(type_key, registration.agent_id)
    ...
    def get_agents_by_type(self, agent_type: str) -> List[str]:
        ...
        try:
            ...
            return [agent_id.decode() for agent_id in self.redis.smembers(type_key)]
        except Exception as e:
    ...
    def get_conversation_history(self, conversation_id: UUID, limit: int = 50) -> List[A2AMessage]:
        """Get conversation message history."""
        try:
            history_key = f"conversation:{conversation_id}:messages"
            raw_messages = self.redis.lrange(history_key, 0, limit - 1)
    ...
    def cleanup_expired_messages(self) -> int:
        ...
        try:
            ...
            for key in self.redis.scan_iter(match=pattern):
                ...
                if agent_data:
                    registration = AgentRegistration(**json.loads(agent_data))
                    if datetime.utcnow() - registration.last_heartbeat > timedelta(minutes=10):
                        self.unregister_agent(registration.agent_id)
                        cleaned += 1
