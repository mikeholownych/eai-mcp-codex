...
class PlannerAgent(BaseAgent):
    ...
    async def _break_down_project(self, description: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "estimated_total_hours": sum(task["estimated_hours"] for task in enhanced_tasks),
            "tasks": enhanced_tasks,
            "recommendations": await self._generate_recommendations(enhanced_tasks, context)
        }
    ...
    async def _estimate_effort(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "confidence_level": sum(est["confidence"] for est in task_estimates) / len(task_estimates),
    ...
    async def _create_timeline(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        tasks = context.get("tasks", [])
        start_date = datetime.fromisoformat(context.get("start_date", datetime.utcnow().isoformat()))
        team_size = context.get("team_size", 3)
        ...
        for task in sorted_tasks:
            estimated_hours = task.get("estimated_hours", 8)
            working_days = max(1, estimated_hours / (6 * team_size))  # 6 productive hours per day
    ...
    async def _create_milestones(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        tasks = context.get("tasks", [])
        project_phases = context.get("phases", ["planning", "development", "testing", "deployment"])
    ...
    async def _web_app_template(self, description: str, complexity: str) -> List[Dict[str, Any]]:
    ...
    async def _api_service_template(self, description: str, complexity: str) -> List[Dict[str, Any]]:
    ...
    async def _data_pipeline_template(self, description: str, complexity: str) -> List[Dict[str, Any]]:
    ...
    async def _mobile_app_template(self, description: str, complexity: str) -> List[Dict[str, Any]]:
    ...
    async def _general_breakdown(self, description: str, complexity: str) -> List[Dict[str, Any]]:
    ...
    def _topological_sort(self, tasks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        while remaining_tasks:
            ...
            for task in remaining_tasks:
                ...
                if all(any(t.get("id") == dep for t in sorted_tasks) or dep not in [t.get("id") for t in tasks] for dep in deps):
    ...
    def _is_critical_path(self, task: Dict[str, Any], all_tasks: List[Dict[str, Any]]) -> bool:
    ...
    def _extract_milestones(self, timeline: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        milestone_indices = [int(total_items * 0.25), int(total_items * 0.5), int(total_items * 0.75), total_items - 1]
    ...
    def _has_circular_dependency(self, task_id: str, dependency_graph: Dict[str, List[str]]) -> bool:
        """Check for circular dependencies."""
        visited = set()
        path = set()
    ...
    def _generate_dependency_recommendations(self, dependency_graph: Dict[str, List[str]], issues: List[Dict[str, Any]]) -> List[str]:
        ...
        if any(len(deps) > 5 for deps in dependency_graph.values()):
            recommendations.append("Some tasks have too many dependencies - consider simplification")
        
        return recommendations
    
    def _group_tasks_by_phase(self, tasks: List[Dict[str, Any]], phases: List[str]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def _get_phase_criteria(self, phase: str) -> List[str]:
        ...
        criteria_map = {
            "planning": ["Requirements documented", "Architecture defined", "Timeline approved"],
            "development": ["All features implemented", "Code reviewed", "Unit tests passing"],
            "testing": ["All tests passing", "Performance verified", "Security validated"],
            "deployment": ["Production deployment successful", "Monitoring active", "Documentation complete"]
        }
    ...
    def _get_phase_deliverables(self, phase: str) -> List[str]:
        ...
        deliverables_map = {
            "planning": ["Requirements document", "Architecture diagram", "Project plan"],
            "development": ["Source code", "API documentation", "Development environment"],
            "testing": ["Test results", "Performance report", "Bug reports"],
            "deployment": ["Production system", "Deployment guide", "User documentation"]
        }
    ...
    def _identify_milestone_risks(self, milestones: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def _generate_recommendations(self, tasks: List[Dict[str, Any]], context: Dict[str, Any]) -> List[str]:
        ...
        if total_hours > 200:
            recommendations.append("Consider breaking this into multiple phases or sprints")
        ...
        if len(all_skills) > 8:
            recommendations.append("Project requires diverse skill set - ensure team coverage")
        
        # Check for high-risk tasks
        high_risk_tasks = [t for t in tasks if t.get("priority") == "high" and t["estimated_hours"] > 20]
        if high_risk_tasks:
            recommendations.append("Consider breaking down high-priority, high-effort tasks")
