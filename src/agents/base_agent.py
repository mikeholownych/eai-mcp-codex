...
from src.a2a_communication.models import (
    A2AMessage, AgentRegistration, AgentStatus, MessageType, AgentCapabilities
)
...
class BaseAgent(ABC):
    ...
    async def register(self) -> bool:
        ...
        try:
            ...
            if success:
                self.logger.info(f"Agent {self.config.agent_id} registered successfully")
            return success
    ...
    async def unregister(self) -> bool:
        ...
        try:
            ...
            if success:
                self.logger.info(f"Agent {self.config.agent_id} unregistered successfully")
            return success
    ...
    async def _task_processor(self) -> None:
        ...
        while self.running:
            try:
                ...
                while (len(self.current_tasks) < self.config.max_concurrent_tasks and 
                       self.task_queue):
        ...
        try:
            ...
            response = A2AMessage(
                ...
                payload={
                    ...
                    "capabilities": self.config.capabilities,
                    "message": "Available for collaboration" if can_participate else "Currently at capacity"
                },
                conversation_id=message.conversation_id
            )
            await self.message_broker.send_message(response)
    ...
    async def _handle_notification(self, message: A2AMessage) -> None:
        ...
        self.logger.info(f"Notification: {message.payload.get('message', 'No message')}")
    ...
    async def _can_participate_in_collaboration(self, payload: Dict[str, Any]) -> bool:
        ...
        has_capabilities = all(cap in self.config.capabilities for cap in required_capabilities)
