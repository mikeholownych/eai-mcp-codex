...
class RetryHandler:
    ...
    def _classify_fault(self, exception: Exception) -> FaultType:
        ...
        elif any(keyword in error_msg for keyword in ['unavailable', '503', 'service down']):
            return FaultType.SERVICE_UNAVAILABLE
        elif any(keyword in error_msg for keyword in ['auth', 'unauthorized', '401', '403']):
...
class GracefulDegradationManager:
    ...
    async def execute_with_degradation(self, service_name: str, primary_func: Callable, 
                                     *args, **kwargs) -> Any:
        ...
        if degradation_level == 0:
            ...
            logger.info(f"Using degraded mode for {service_name} (level {degradation_level})")
            return await self._execute_fallback(service_name, *args, **kwargs)
...
class FaultTolerantExecutor:
    ...
    async def execute_task(self, task: Task, executor_func: Callable, 
                          circuit_breaker_name: Optional[str] = None,
                          retry_config: Optional[RetryConfig] = None) -> ExecutionResult:
        ...
        try:
            ...
            if circuit_breaker_name:
                ...
                result = await cb.call(self._execute_with_retry, executor_func, task, retry_config)
            else:
                result = await self._execute_with_retry(executor_func, task, retry_config)
            ...
            execution_result = ExecutionResult(
                ...
                metadata={'circuit_breaker': circuit_breaker_name, 'traceback': traceback.format_exc()}
            )
    ...
    async def _execute_with_retry(self, executor_func: Callable, task: Task, 
    ...
    async def execute_plan_with_fault_tolerance(self, plan: Plan, 
                                              task_executors: Dict[str, Callable]) -> List[ExecutionResult]:
        ...
        for task in plan.tasks:
            ...
            result = await self.execute_task(
                task, 
                executor_func, 
                circuit_breaker_name=f"task_{task_type}"
            )
            ...
            if not result.success:
                ...
                if await self._should_continue_on_failure(task, result, plan):
                    logger.warning(f"Continuing plan execution despite task {task.id} failure")
                else:
                    logger.error(f"Stopping plan execution due to critical task {task.id} failure")
                    break
        
        # Attempt recovery for failed tasks
        if failed_tasks:
            recovery_results = await self._attempt_task_recovery(failed_tasks, task_executors)
            results.extend(recovery_results)
        
        logger.info(f"Plan {plan.id} execution completed. Success: {sum(1 for r in results if r.success)}/{len(results)}")
        return results
    
    async def _should_continue_on_failure(self, task: Task, result: ExecutionResult, plan: Plan) -> bool:
        ...
        # Check fault type
        if result.fault_type in [FaultType.VALIDATION_ERROR, FaultType.AUTHENTICATION_ERROR]:
            return False
    ...
    async def _attempt_task_recovery(self, failed_tasks: List[Task], 
                                   task_executors: Dict[str, Callable]) -> List[ExecutionResult]:
    ...
    def get_execution_statistics(self) -> Dict[str, Any]:
        ...
        recovery_rate = (self.execution_stats['recovered_executions'] / max(self.execution_stats['failed_executions'], 1) * 100)
        
        # Fault type distribution
        fault_distribution = {}
        for result in self.execution_history:
            if result.fault_type:
                fault_distribution[result.fault_type.value] = fault_distribution.get(result.fault_type.value, 0) + 1
        ...
        return {
            ...
            'recent_executions': len([r for r in self.execution_history if 
                                    datetime.utcnow() - datetime.fromisoformat(r.metadata.get('timestamp', '2024-01-01T00:00:00')) < timedelta(hours=1)])
        }
...
fault_tolerant_executor.degradation_manager.register_fallback("model_router", default_model_fallback)
fault_tolerant_executor.degradation_manager.register_fallback("database", default_database_fallback)
