...
from src.common.fastapi_auth import verify_staff_access, get_current_user, require_admin_access
...
async def update_user(
    ...
    logger.info(f"User {user_id} update requested by {current_user.get('email', 'unknown')}")
...
async def delete_user(
    ...
    logger.info(f"User {user_id} deletion requested by {current_user.get('email', 'unknown')}")
...
async def suspend_user(
    ...
    logger.info(f"User {user_id} suspension requested by {current_user.get('email', 'unknown')}")
...
async def activate_user(
    ...
    logger.info(f"User {user_id} activation requested by {current_user.get('email', 'unknown')}")
...
async def list_tickets(
    ...
    if search:
        ...
        filtered_tickets = [t for t in filtered_tickets if 
                           ...
                           search_lower in t.customer["email"].lower() or
...
async def get_ticket(
    ...
    logger.info(f"Ticket {ticket_id} requested by {current_user.get('email', 'unknown')}")
...
async def create_ticket(
    ...
    try:
        ...
        import time
        new_ticket_id = f"T-{datetime.now().strftime('%Y')}-{int(time.time() % 10000):04d}"
...
async def update_ticket(
    ...
    logger.info(f"Ticket {ticket_id} update requested by {current_user.get('email', 'unknown')}")
    
    try:
        # Check if ticket exists (mock)
        valid_tickets = ["T-2024-001", "T-2024-002", "T-2024-003", "T-2024-004", "T-2024-005"]
...
async def assign_ticket(
    ...
    logger.info(f"Ticket {ticket_id} assignment requested by {current_user.get('email', 'unknown')}")
...
async def update_ticket_status(
    ...
    logger.info(f"Ticket {ticket_id} status update requested by {current_user.get('email', 'unknown')}")
...
async def update_ticket_priority(
    ...
    logger.info(f"Ticket {ticket_id} priority update requested by {current_user.get('email', 'unknown')}")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to update ticket priority: {str(e)}")
