...
class StaffSettings(BaseSettings):
    ...
    staff_roles: list[str] = Field(
        default=["admin", "manager", "support"], env="STAFF_ALLOWED_ROLES"
    )
    ...
    enable_user_management: bool = Field(
        default=True, env="STAFF_ENABLE_USER_MANAGEMENT"
    )
    enable_ticket_management: bool = Field(
        default=True, env="STAFF_ENABLE_TICKET_MANAGEMENT"
    )
    enable_system_monitoring: bool = Field(
        default=True, env="STAFF_ENABLE_SYSTEM_MONITORING"
    )
