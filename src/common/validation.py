...
def validate_email(email: str, field_name: str = "email") -> None:
    ...
    if not re.match(email_pattern, email):
        raise ValidationError(f"Invalid email format for {field_name}", field=field_name)
...
def validate_path(path: str, field_name: str = "path", must_exist: bool = False) -> None:
    ...
    try:
        ...
        raise ValidationError(
            f"Invalid path format for {field_name}: {str(e)}",
            field=field_name
        )
    ...
    if allowed_keys:
        ...
        if invalid_keys:
            raise ValidationError(
                ...
                details={"invalid_keys": list(invalid_keys), "allowed_keys": allowed_keys}
            )
...
class ValidationSchema:
    ...
    def validate(self, data: Dict[str, Any]) -> None:
        ...
        required_fields = [
            field for field, rule in self.rules.items() 
            if rule['required']
        ]
...
def create_user_validation_schema() -> ValidationSchema:
    ...
    return (ValidationSchema()
            .add_field('username', required=True, validator=validate_string_length, min_length=3, max_length=50)
...
def create_task_validation_schema() -> ValidationSchema:
    ...
    return (ValidationSchema()
            .add_field('title', required=True, validator=validate_string_length, min_length=1, max_length=200)
            .add_field('description', required=False, validator=validate_string_length, max_length=1000)
            .add_field('priority', required=False, validator=validate_choice, choices=['low', 'medium', 'high'])
