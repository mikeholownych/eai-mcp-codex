...
from .models import (
    CollaborationSession, CollaborationStatus, CollaborationInvitation,
    CollaborationResponse, ConsensusDecision, WorkflowStep, DecisionType,
    VoteChoice, ParticipantRole, EscalationRequest, CollaborationMetrics
)
...
class CollaborationOrchestrator:
    ...
    async def invite_agent_to_collaboration(
        ...
        logger.info(f"Sent collaboration invitation to {agent_id} for session {session_id}")
        return invitation
    ...
    async def respond_to_invitation(
        ...
        logger.info(f"Agent {agent_id} {'accepted' if accepted else 'declined'} invitation {invitation_id}")
        return True
    ...
    async def start_collaboration(self, session_id: UUID) -> bool:
        ...
        await self._notify_participants(
            session,
            f"Collaboration '{session.title}' has started!"
        )
    ...
    async def escalate_issue(
        ...
        escalation = EscalationRequest(
            ...
            priority=priority,
            resolution_deadline=datetime.utcnow() + timedelta(hours=4)  # 4 hours to resolve
        )
    ...
    async def complete_collaboration(self, session_id: UUID, outputs: Dict = None) -> bool:
        ...
        await self._notify_participants(
            session,
            f"Collaboration '{session.title}' has been completed successfully!"
        )
    ...
    async def _check_consensus(self, decision: ConsensusDecision) -> bool:
        ...
        approve_votes = sum(1 for vote in decision.votes.values() if vote == VoteChoice.APPROVE)
        ...
        reject_votes = sum(1 for vote in decision.votes.values() if vote == VoteChoice.REJECT)
    ...
    async def _calculate_session_metrics(self, session: CollaborationSession) -> CollaborationMetrics:
        """Calculate metrics for a completed collaboration session."""
        # Get conversation history
        messages = await self.message_broker.get_conversation_history(session.session_id)
        
        # Count decisions made
        decisions_made = len([d for d in self.consensus_decisions.values() 
                             if d.session_id == session.session_id])
        consensus_achieved = len([d for d in self.consensus_decisions.values() 
                                 if d.session_id == session.session_id and d.resolved and d.resolution == "approved"])
