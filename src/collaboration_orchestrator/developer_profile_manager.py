...
class DeveloperProfileManager:
    ...
    async def get_developer_profile(self, agent_id: str) -> Optional[DeveloperProfile]:
        ...
        async with self.db_manager.get_connection() as conn:
            ...
            profile_data['specializations'] = [
                DeveloperSpecialization(s) for s in json.loads(profile_data['specializations'] or '[]')
            ]
            profile_data['programming_languages'] = json.loads(profile_data['programming_languages'] or '[]')
    ...
    async def update_performance_metrics(
        ...
        if completion_time:
            if metrics.average_completion_time:
                ...
                metrics.average_completion_time = (
                    (metrics.average_completion_time * (total_tasks - 1) + completion_time) / total_tasks
                )
            else:
        ...
        if quality_score:
            metrics.code_quality_score = (metrics.code_quality_score + quality_score) / 2
        ...
        # Update profile
        return await self.update_developer_profile(agent_id, {"performance_metrics": metrics})
    ...
    async def get_agent_workload(self, agent_id: str) -> Optional[DeveloperWorkload]:
        ...
        async with self.db_manager.get_connection() as conn:
            ...
            max_hours = profile.max_concurrent_tasks * 8  # Assuming 8 hours per task slot
            utilization_percentage = (total_estimated_hours / max_hours) * 100 if max_hours > 0 else 0
            ...
            if any(deadline[1] < datetime.utcnow() + timedelta(days=1) for deadline in upcoming_deadlines):
    ...
    async def get_collaboration_history(
        ...
        async with self.db_manager.get_connection() as conn:
            ...
            history_data['quality_ratings'] = json.loads(history_data['quality_ratings'] or '[]')
            history_data['improvement_areas'] = json.loads(history_data['improvement_areas'] or '[]')
    ...
    async def update_collaboration_history(
        ...
        async with self.db_manager.get_connection() as conn:
            ...
            """,
                str(history.collaboration_id), str(history.session_id), 
                history.agent_1, history.agent_2, history.collaboration_type,
                history.interaction_count, history.successful_collaborations, history.conflict_count,
                history.average_response_time, json.dumps(history.quality_ratings),
                history.communication_effectiveness, history.task_completion_rate,
                history.mutual_trust_score, history.collaboration_notes,
    ...
    async def get_available_agents(
        ...
        for agent_id in agent_ids:
            ...
            if min_experience:
                ...
                if experience_levels.index(profile.experience_level) < experience_levels.index(min_experience):
                    continue
