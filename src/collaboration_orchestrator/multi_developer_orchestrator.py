...
class TaskAssignmentEngine:
    ...
    async def optimize_task_assignments(
        ...
        tasks.sort(key=lambda t: (t.priority == "high", t.complexity_score), reverse=True)
    ...
    async def _calculate_assignment_score(
        ...
        if task.complexity_score > 0.7:
            experience_levels = [ExperienceLevel.JUNIOR, ExperienceLevel.INTERMEDIATE, 
                               ExperienceLevel.SENIOR, ExperienceLevel.LEAD, ExperienceLevel.ARCHITECT]
            experience_bonus = experience_levels.index(agent.experience_level) * 0.05
        
        total_score = capability_score + performance_bonus + preference_bonus + experience_bonus - workload_penalty
        return max(0.0, min(1.0, total_score))
    
    def suggest_task_optimizations(
        self, 
        plan: TeamCoordinationPlan,
        agent_profiles: Dict[str, DeveloperProfile]
        ...
        for assignment_id, assignment in plan.task_assignments.items():
            ...
            workload = plan.get_team_workload_distribution().get(assignment.assigned_agent, 0)
            if workload > current_agent.max_concurrent_tasks * 6:  # 75% capacity
                suggestions.append(TaskOptimizationSuggestion(
            ...
            capability_score = current_agent.get_capability_score(assignment.task_type, required_skills)
    ...
    def _suggest_parallelization(self, plan: TeamCoordinationPlan) -> List[TaskOptimizationSuggestion]:
        ...
        for task in tasks:
            if len(task.dependencies) == 0 and task.status == TaskStatus.PENDING:
                ...
                similar_tasks = [
                    t for t in tasks 
                    if t.task_type == task.task_type and t.assignment_id != task.assignment_id
                ]
...
class MultiDeveloperOrchestrator(CollaborationOrchestrator):
    ...
    async def create_team_coordination_plan(
        ...
        for i, task_data in enumerate(tasks):
            assignment = TaskAssignment(
                ...
                task_type=TaskType(task_data.get("type", TaskType.FEATURE_DEVELOPMENT.value)),
        ...
        logger.info(f"Created team coordination plan {plan.plan_id} for project: {project_name}")
        return plan
    ...
    async def handle_task_progress_update(
        ...
        if status == TaskStatus.IN_PROGRESS and not assignment.started_at:
            ...
            if assignment.started_at:
                actual_effort = (assignment.completed_at - assignment.started_at).total_seconds() / 3600
        ...
        logger.info(f"Updated task progress: {assignment.task_name} - {progress_percentage}%")
        return True
    ...
    async def detect_and_resolve_conflicts(self, plan_id: UUID) -> List[Dict[str, Any]]:
        ...
        for conflict_data in detected_conflicts:
            ...
            # Attempt resolution
            resolved = await self.conflict_resolver.resolve_conflict(conflict.conflict_id)
    ...
    async def _detect_resource_conflicts(self, plan: TeamCoordinationPlan) -> List[Dict[str, Any]]:
            ...
            if len(assignments) > 1:
                ...
                if overlapping_assignments:
                    conflicts.append({
                        ...
                        "involved_agents": list(set(a.assigned_agent for a in overlapping_assignments)),
                        "context": {
                            "resource": resource,
                            "conflicting_tasks": [a.task_name for a in overlapping_assignments],
                            "affects_critical_path": any(a.priority == "high" for a in overlapping_assignments)
                        }
                    })
        
        return conflicts
    
    async def _detect_timeline_conflicts(self, plan: TeamCoordinationPlan) -> List[Dict[str, Any]]:
        ...
        overdue_tasks = [
            assignment for assignment in plan.task_assignments.values()
            if assignment.deadline and assignment.deadline < current_time and 
               assignment.status not in [TaskStatus.COMPLETED, TaskStatus.CANCELLED]
        ]
        ...
        for assignment in plan.task_assignments.values():
            if assignment.deadline and assignment.status == TaskStatus.PENDING:
                remaining_time = (assignment.deadline - current_time).total_seconds() / 3600
    ...
    async def _detect_dependency_conflicts(self, plan: TeamCoordinationPlan) -> List[Dict[str, Any]]:
        ...
        for assignment in plan.task_assignments.values():
            for dep_id in assignment.dependencies:
                ...
                if self._has_circular_dependency(assignment, dep_assignment, plan.task_assignments):
                    conflicts.append({
                        "description": f"Circular dependency detected involving {assignment.task_name}",
                        "involved_agents": [assignment.assigned_agent, dep_assignment.assigned_agent],
                        "context": {
                ...
                if (dep_assignment.status == TaskStatus.BLOCKED and 
    ...
    def _assignments_overlap(self, assignment1: TaskAssignment, assignment2: TaskAssignment) -> bool:
    ...
    async def _handle_task_blockers(self, assignment: TaskAssignment, blockers: List[str]) -> None:
        """Handle task blockers by creating conflict entries."""
        for blocker in blockers:
            await self.conflict_resolver.detect_conflict(
                session_id=UUID(str(assignment.plan_id)),  # Use plan_id as session_id for now
    ...
    async def get_coordination_plan(self, plan_id: UUID) -> Optional[TeamCoordinationPlan]:
        ...
        try:
            row = await conn.fetchrow(
                "SELECT * FROM team_coordination_plans WHERE plan_id = $1",
                str(plan_id)
            )
            if not row:
                return None
            ...
            plan_data['communication_plan'] = json.loads(plan_data['communication_plan'] or '{}')
            plan_data['conflict_resolution_strategy'] = ResolutionStrategy(plan_data['conflict_resolution_strategy'])
            plan_data['success_metrics'] = json.loads(plan_data['success_metrics'] or '{}')
            plan_data['risk_factors'] = json.loads(plan_data['risk_factors'] or '[]')
            plan_data['milestone_schedule'] = json.loads(plan_data['milestone_schedule'] or '[]')
            plan_data['resource_requirements'] = json.loads(plan_data['resource_requirements'] or '{}')
            ...
            task_rows = await conn.fetch(
                "SELECT * FROM task_assignments WHERE plan_id = $1",
                str(plan_id)
            )
            ...
            for task_row in task_rows:
                ...
                assignment_data['reviewer_agents'] = json.loads(assignment_data['reviewer_agents'] or '[]')
                assignment_data['dependencies'] = json.loads(assignment_data['dependencies'] or '[]')
                assignment_data['requirements'] = json.loads(assignment_data['requirements'] or '{}')
                assignment_data['deliverables'] = json.loads(assignment_data['deliverables'] or '[]')
                assignment_data['feedback'] = json.loads(assignment_data['feedback'] or '[]')
                assignment_data['blockers'] = json.loads(assignment_data['blockers'] or '[]')
                assignment_data['metadata'] = json.loads(assignment_data['metadata'] or '{}')
    ...
    async def _get_available_team_agents(self, team_member_ids: List[str]) -> List[DeveloperProfile]:
    ...
    async def _store_coordination_plan(self, plan: TeamCoordinationPlan) -> None:
        ...
        try:
            ...
            """,
                str(plan.plan_id), str(plan.session_id), plan.project_name, plan.project_description,
                plan.team_lead, json.dumps(plan.team_members), json.dumps({}),  # task_assignments stored separately
                json.dumps(plan.dependencies), json.dumps(plan.communication_plan.model_dump()),
                plan.conflict_resolution_strategy.value, plan.status, plan.priority,
                plan.estimated_duration, json.dumps(plan.success_metrics),
                json.dumps(plan.risk_factors), json.dumps(plan.milestone_schedule),
                json.dumps(plan.resource_requirements), json.dumps(plan.metadata),
                plan.deadline, plan.created_at, plan.updated_at
            )
    ...
    async def _store_task_assignment(self, assignment: TaskAssignment, conn: Optional[asyncpg.Connection] = None) -> None:
        ...
        try:
            ...
            """,
                str(assignment.assignment_id), str(assignment.plan_id), assignment.task_name,
                assignment.task_description, assignment.task_type.value, assignment.assigned_agent,
                json.dumps(assignment.reviewer_agents), json.dumps([str(d) for d in assignment.dependencies]),
                json.dumps(assignment.requirements), json.dumps(assignment.deliverables),
                assignment.status.value, assignment.priority, assignment.complexity_score,
                assignment.estimated_effort, assignment.actual_effort, assignment.progress_percentage,
                assignment.quality_score, json.dumps(assignment.feedback), json.dumps(assignment.blockers),
                json.dumps(assignment.metadata), assignment.deadline, assignment.created_at,
                assignment.updated_at, assignment.started_at, assignment.completed_at
            )
        finally:
    ...
    async def _update_coordination_plan(self, plan: TeamCoordinationPlan) -> None:
        ...
        try:
            await conn.execute("""
                ...
                WHERE plan_id = $1
            """,
                str(plan.plan_id), json.dumps(plan.team_members), json.dumps(plan.dependencies),
                json.dumps(plan.communication_plan.model_dump()), plan.status, plan.priority,
                plan.estimated_duration, plan.actual_duration, json.dumps(plan.success_metrics),
                json.dumps(plan.risk_factors), json.dumps(plan.milestone_schedule),
                json.dumps(plan.resource_requirements), json.dumps(plan.metadata),
    ...
    async def _get_task_assignment(self, assignment_id: UUID) -> Optional[TaskAssignment]:
        ...
        try:
            ...
            assignment_data['status'] = TaskStatus(assignment_data['status'])
            assignment_data['reviewer_agents'] = json.loads(assignment_data['reviewer_agents'] or '[]')
            assignment_data['dependencies'] = [UUID(d) for d in json.loads(assignment_data['dependencies'] or '[]')]
            assignment_data['requirements'] = json.loads(assignment_data['requirements'] or '{}')
            assignment_data['deliverables'] = json.loads(assignment_data['deliverables'] or '[]')
            assignment_data['feedback'] = json.loads(assignment_data['feedback'] or '[]')
            assignment_data['blockers'] = json.loads(assignment_data['blockers'] or '[]')
    ...
    async def generate_team_performance_report(self, plan_id: UUID) -> Dict[str, Any]:
        ...
        completed_tasks = len([a for a in plan.task_assignments.values() if a.status == TaskStatus.COMPLETED])
        in_progress_tasks = len([a for a in plan.task_assignments.values() if a.status == TaskStatus.IN_PROGRESS])
        blocked_tasks = len([a for a in plan.task_assignments.values() if a.status == TaskStatus.BLOCKED])
        
        # Calculate team workload distribution
        workload_distribution = plan.get_team_workload_distribution()
        
        # Get conflict statistics
        conflict_stats = await self.conflict_resolver.get_conflict_statistics(plan.session_id)
        
        # Calculate individual agent performance
        agent_performance = {}
        for agent_id in plan.team_members:
            agent_tasks = [a for a in plan.task_assignments.values() if a.assigned_agent == agent_id]
            agent_completed = len([a for a in agent_tasks if a.status == TaskStatus.COMPLETED])
            agent_performance[agent_id] = {
                "total_tasks": len(agent_tasks),
                "completed_tasks": agent_completed,
                "completion_rate": agent_completed / len(agent_tasks) if agent_tasks else 0,
        ...
        optimization_suggestions = self.assignment_engine.suggest_task_optimizations(plan, agent_profiles)
        ...
        return {
            ...
            "optimization_suggestions": [s.model_dump() for s in optimization_suggestions],
