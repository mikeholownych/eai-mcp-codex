...
class DeveloperProfile(BaseModel):
    ...
    trust_scores: Dict[str, float] = Field(default_factory=dict)  # agent_id -> trust_score
    metadata: Dict[str, Any] = Field(default_factory=dict)
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: datetime = Field(default_factory=datetime.utcnow)
    
    def get_capability_score(self, task_type: TaskType, required_skills: List[str]) -> float:
        ...
        # Skill match bonus
        skill_matches = len(set(required_skills) & set(self.programming_languages + self.frameworks))
...
class TaskAssignment(BaseModel):
    ...
    def is_overdue(self) -> bool:
        ...
        return self.deadline and datetime.utcnow() > self.deadline and self.status != TaskStatus.COMPLETED
...
class TeamCoordinationPlan(BaseModel):
    ...
    task_assignments: Dict[str, TaskAssignment] = Field(default_factory=dict)  # assignment_id -> assignment
    dependencies: Dict[str, List[str]] = Field(default_factory=dict)  # task_id -> [dependency_task_ids]
    communication_plan: "CommunicationPlan" = Field(default_factory=lambda: CommunicationPlan())
    ...
    def completion_percentage(self) -> float:
        ...
        if not self.task_assignments:
            return 0.0
        total_progress = sum(assignment.progress_percentage for assignment in self.task_assignments.values())
...
class AgentCollaborationHistory(BaseModel):
    ...
    def update_trust_score(self, success: bool, quality_rating: Optional[float] = None) -> None:
        ...
        avg_quality = sum(self.quality_ratings) / len(self.quality_ratings) if self.quality_ratings else 0.5
        
        # Weight recent interactions more heavily
        recency_factor = min(1.0, self.interaction_count / 10)
        self.mutual_trust_score = (success_rate * 0.6 + avg_quality * 0.4) * recency_factor
...
class TaskOptimizationSuggestion(BaseModel):
    ...
    expected_improvement: Dict[str, float] = Field(default_factory=dict)  # metric -> improvement
    rationale: str
...
class DeveloperWorkload(BaseModel):
    ...
    availability_forecast: Dict[str, float] = Field(default_factory=dict)  # date -> availability_percentage
