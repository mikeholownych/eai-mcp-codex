...
class FeatureExtractor:
    ...
    def extract_features(self, plan: Plan) -> Dict[str, float]:
        ...
        # Basic metrics
        features['task_count'] = len(plan.tasks) if hasattr(plan, 'tasks') else 0
        features['estimated_hours'] = plan.estimated_hours or 0
        ...
        features['integration_complexity'] = self._calculate_integration_complexity(plan)
        
        # Text-based features
        features['description_length'] = len(plan.description)
        features['title_complexity'] = len(plan.title.split())
        
        # Risk indicators
        features['risk_keywords'] = self._count_risk_keywords(plan.description)
        features['technical_debt_indicators'] = self._count_technical_debt_keywords(plan.description)
        
        # Timeline features
        if plan.start_date and plan.end_date:
            features['planned_duration'] = (plan.end_date - plan.start_date).total_seconds() / 3600
        else:
            features['planned_duration'] = features['estimated_hours']
        
        # Derived features
        features['velocity_indicator'] = features['task_count'] / max(features['planned_duration'], 1)
        features['scope_density'] = features['description_length'] / max(features['task_count'], 1)
    ...
    def _calculate_complexity_score(self, plan: Plan) -> float:
        ...
        complexity_keywords = [
            'algorithm', 'optimization', 'machine learning', 'ai', 'blockchain',
            'microservices', 'distributed', 'real-time', 'concurrent', 'parallel',
            'security', 'encryption', 'compliance', 'integration', 'migration'
        ]
        
        keyword_count = sum(1 for keyword in complexity_keywords if keyword in description)
    ...
    def _calculate_integration_complexity(self, plan: Plan) -> float:
        ...
        integration_keywords = [
            ...
            'database', 'cloud', 'aws', 'azure', 'gcp', 'webhook'
        ]
        
        description = plan.description.lower()
        integration_count = sum(1 for keyword in integration_keywords if keyword in description)
    ...
    def _count_risk_keywords(self, text: str) -> float:
        ...
        risk_keywords = [
            'legacy', 'deprecated', 'experimental', 'beta', 'unknown',
    ...
    def _count_technical_debt_keywords(self, text: str) -> float:
        ...
        debt_keywords = [
            'refactor', 'cleanup', 'technical debt', 'legacy code',
            'workaround', 'hack', 'temporary', 'quick fix'
        ]
...
class TimeSeriesPredictor:
    ...
    def _calculate_confidence(self, features: Dict[str, float]) -> float:
        ...
        for data_point in self.historical_data:
            similarity = self._calculate_feature_similarity(features, data_point.features)
...
class AnomalyDetectionSystem:
    ...
    def detect_anomalies(self, plan: Plan, features: Dict[str, float], 
                        prediction: PredictionResult, 
                        historical_data: List[HistoricalDataPoint]) -> List[AnomalyDetection]:
    ...
    def _detect_duration_anomaly(self, prediction: PredictionResult, 
                                historical_data: List[HistoricalDataPoint]) -> Optional[AnomalyDetection]:
    ...
    def _detect_complexity_anomaly(self, features: Dict[str, float]) -> Optional[AnomalyDetection]:
    ...
    def _detect_risk_anomaly(self, features: Dict[str, float]) -> Optional[AnomalyDetection]:
        """Detect high risk indicators."""
        risk_score = features.get('risk_keywords', 0) + features.get('technical_debt_indicators', 0)
        
        if risk_score > self.thresholds['risk_score_threshold']:
            return AnomalyDetection(
                is_anomaly=True,
                anomaly_score=min(risk_score / self.thresholds['risk_score_threshold'], 5.0),
    ...
    def _detect_scope_anomaly(self, features: Dict[str, float]) -> Optional[AnomalyDetection]:
...
class PerformancePredictionEngine:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for row in cursor.fetchall():
                try:
                    ...
                    logger.warning(f"Failed to parse historical data for project {row[0]}: {e}")
            
            if historical_data:
                self.ml_model.train(historical_data)
                logger.info(f"Loaded {len(historical_data)} historical projects for training")
            ...
            for anomaly in anomalies:
                if anomaly.is_anomaly:
                    prediction_result.risk_factors.append({
                        'type': 'anomaly',
                        'level': 'high' if anomaly.anomaly_score > 2.0 else 'medium',
    ...
    def _identify_risk_factors(self, plan: Plan, features: Dict[str, float]) -> List[Dict[str, Any]]:
    ...
    def _calculate_resource_requirements(self, duration: float, 
        ...
        skill_level = "senior" if complexity_score > 0.7 else "mid" if complexity_score > 0.4 else "junior"
    ...
    def _generate_milestone_predictions(self, duration: float, 
        ...
        for milestone in milestones:
            milestone_hours = duration * milestone['percentage']
            ...
            milestone_predictions.append({
                ...
                'confidence': 0.8 - (complexity_score * 0.2)  # Lower confidence for complex projects
            })
    ...
    def _find_similar_projects(self, features: Dict[str, float]) -> List[str]:
        ...
        for data_point in self.ml_model.historical_data:
            similarity = self.ml_model._calculate_feature_similarity(features, data_point.features)
    ...
    async def _store_prediction(self, plan_id: str, prediction: PredictionResult, 
                              features: Dict[str, float]):
    ...
    async def record_actual_duration(self, plan_id: str, actual_duration: float):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                logger.info(f"Recorded actual duration {actual_duration}h for plan {plan_id}")
        except Exception as e:
