...
async def execute_workflow_engine(
    workflow_id: str, context: Dict[str, Any] = None
) -> WorkflowExecution:
    ...
    try:
        ...
        execution = await orchestrator.execute_workflow(
            workflow_id, triggered_by="workflow_engine", context=context
        )
        logger.info(
            f"Workflow {workflow_id} execution completed with status: {execution.status}"
        )
        return execution
...
def run(workflow: Workflow) -> str:
    ...
    logger.warning(
        f"Legacy run function called for workflow {workflow.id}. Consider using execute_workflow_engine."
    )
    ...
    try:
        ...
        execution = asyncio.run(
            orchestrator.execute_workflow(workflow.id, triggered_by="legacy_run")
        )
...
def status(workflow: Workflow) -> str:
    ...
    logger.warning(
        f"Legacy status function called for workflow {workflow.id}. Consider querying orchestrator directly."
    )
    orchestrator = get_orchestrator()
