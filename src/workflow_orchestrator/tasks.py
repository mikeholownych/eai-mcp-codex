...
async def schedule_workflow_execution(workflow_id: str, triggered_by: str = "system", context: Dict[str, Any] = None) -> WorkflowExecution:
    ...
    try:
        ...
        execution = await orchestrator.execute_workflow(workflow_id, triggered_by, context)
...
def schedule(workflow_id: str) -> Workflow: # Legacy function for backward compatibility
    ...
    logger.warning(f"Legacy schedule function called for workflow {workflow_id}. Consider using schedule_workflow_execution.")
    ...
    import threading
    threading.Thread(target=asyncio.run, args=(schedule_workflow_execution(workflow_id),)).start()
    
    # Return a placeholder Workflow object for immediate response
    from .models import WorkflowStatus # Import here to avoid circular dependency if Workflow is in models.py
    return Workflow(id=workflow_id, name=f"Workflow {workflow_id} (Scheduled)", status=WorkflowStatus.PENDING)