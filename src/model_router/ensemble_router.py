...
class DynamicWeights:
    ...
    def calculate_best_model(self, task: ModelRequest, performance_data: Dict[str, Dict]) -> str:
        ...
        for model_id, base_weight in self.base_weights.items():
            if model_id in performance_data:
                ...
                composite_score = accuracy_score + speed_score + reliability_score + efficiency_score
    ...
    def _assess_complexity(self, task: ModelRequest) -> TaskComplexity:
        ...
        code_keywords = ['function', 'class', 'algorithm', 'optimize', 'refactor', 'debug']
        if any(keyword in task.messages[0].get('content', '').lower() for keyword in code_keywords):
            complexity_indicators += 1
            
        # Check for critical keywords
        critical_keywords = ['security', 'critical', 'production', 'compliance', 'audit']
        if any(keyword in task.messages[0].get('content', '').lower() for keyword in critical_keywords):
...
class EnsembleModelRouter:
    ...
    async def route_with_consensus(self, task: ModelRequest, confidence_threshold: float = 0.85) -> EnsembleResult:
        ...
        if task_complexity == TaskComplexity.CRITICAL:
            ...
            logger.info(f"Using single model routing for {task_complexity.value} task: {task.request_id}")
    ...
    async def _consensus_routing(self, task: ModelRequest, confidence_threshold: float) -> EnsembleResult:
        ...
        predictions = await self._get_multi_model_predictions(task, list(self.available_models.keys())[:3])
        
        # Calculate consensus
        consensus_response, consensus_confidence = self._calculate_consensus(predictions)
    ...
    async def _get_multi_model_predictions(self, task: ModelRequest, model_ids: List[str]) -> List[ModelPrediction]:
    ...
    async def _get_model_prediction(self, task: ModelRequest, model_id: str) -> ModelPrediction:
    ...
    def _calculate_confidence(self, response: str, model_id: str) -> float:
        ...
        uncertainty_penalty = sum(0.02 for keyword in uncertainty_keywords if keyword in response.lower())
        
        confidence = base_confidence + length_factor + model_factor - uncertainty_penalty
        
        return max(0.0, min(1.0, confidence))
    
    def _calculate_consensus(self, predictions: List[ModelPrediction]) -> Tuple[str, float]:
    ...
    async def _record_model_performance(self, prediction: ModelPrediction, complexity: TaskComplexity):
...
class AdaptiveModelRouter:
    ...
    async def adaptive_route(self, task: ModelRequest) -> ModelResponse:
        ...
        for model_id in self.routing_weights.base_weights.keys():
            current_performance[model_id] = self.performance_tracker.get_current_metrics(model_id)
        
        # Calculate optimal model
        optimal_model = self.routing_weights.calculate_best_model(task, current_performance)
        
        logger.info(f"Adaptive routing selected {optimal_model} for task {task.request_id}")
        ...
        metrics = {
            ...
            'tokens_per_second': response.usage.get('total_tokens', 0) / max(latency, 0.1)
        }
    ...
    def _estimate_response_quality(self, response: str) -> float:
        ...
        error_penalty = sum(0.1 for indicator in error_indicators if indicator in response.lower())
