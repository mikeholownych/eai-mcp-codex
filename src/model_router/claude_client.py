...
class ClaudeClient:
    ...
    async def route_and_send(
        ...
        # Get model config
        config = self.model_configs.get(model, self.model_configs["claude-3-5-haiku-20241022"])
    ...
    def _calculate_complexity(self, text: str, context: Optional[Dict[str, Any]] = None) -> float:
        ...
        complexity_keywords = [
            "analyze", "complex", "detailed", "comprehensive", "architecture",
            "design", "algorithm", "optimization", "security", "performance",
            "integration", "workflow", "collaboration", "planning"
        ]
        
        keyword_count = sum(1 for keyword in complexity_keywords if keyword.lower() in text.lower())
        score += min(0.4, keyword_count * 0.05)
        
        # Context-based complexity
        if context:
            if context.get("task_type") in ["architecture", "security_review", "complex_analysis"]:
                score += 0.3
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            test_request = ClaudeRequest(
                ...
                messages=[ClaudeMessage(role="user", content="Hello, this is a connection test.")],
                max_tokens=50
            )
            
            response = await self.send_message(test_request)
