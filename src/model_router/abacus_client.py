...
from .models import ModelRequest, ModelResponse, LLMResponse
...
class AbacusAIClient:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            logger.warning("ABACUSAI_API_KEY not found. Abacus.ai client will not be available.")
            self._client = None
        else:
            try:
    ...
    def _select_optimal_model(self, text: str, context: Optional[Dict[str, Any]] = None) -> str:
        ...
        high_complexity_indicators = [
            "architecture", "design", "complex analysis", "research", "planning",
            "security audit", "system design", "multi-step reasoning", 
            "mathematical proof", "strategic planning", "comprehensive analysis"
        ]
        ...
        coding_indicators = [
            ...
            "repository", "git", "framework", "library", "database"
        ]
        
        # Creative/conversational indicators - use general purpose models
        creative_indicators = [
            "creative", "story", "poem", "creative writing", "brainstorm",
            "ideation", "marketing", "content", "blog", "social media"
        ]
        
        # Simple task indicators - use efficient models
        simple_indicators = [
            "simple", "basic", "quick", "summary", "list", "format",
            "translate", "define", "explain briefly", "what is"
        ]
        ...
        if context:
            ...
            if priority == "high" or task_type in [
                "architecture_design", "security_analysis", "complex_planning",
    ...
    async def route_and_send(
        ...
        try:
            ...
            response_content = f"[Abacus.ai {selected_model}] Response to: {text[:100]}..."
