...
class CodeAnalyzer:
    ...
    def _analyze_js_code(self, code: str, filename: str) -> CodeContext:
        ...
        complexity_score = sum(
            len(re.findall(rf"\b{indicator}\b", code))
    ...
    def _analyze_generic_code(
        ...
        for line in lines:
            ...
            if any(keyword in line for keyword in ["def ", "function ", "func "]):
                ...
                keyword in line for keyword in ["class ", "struct ", "interface "]
            ):
                words = line.split()
                if len(words) > 1:
...
class AITestGenerator:
    ...
    async def generate_test_suite(self, request: TestGenerationRequest) -> TestSuite:
        ...
        for function_name in request.code_context.functions:
            function_tests = await self._generate_function_tests(function_name, request)
            test_cases.extend(function_tests)

        # Generate class tests
        for class_name in request.code_context.classes:
            class_tests = await self._generate_class_tests(class_name, request)
        ...
        if (
            TestType.PERFORMANCE in request.test_types
            or request.include_performance_tests
        ):
        ...
        logger.info(
            f"Generated {len(test_cases)} tests with {total_coverage:.1f}% estimated coverage"
        )
        return test_suite

    async def _generate_function_tests(
        self, function_name: str, request: TestGenerationRequest
    ) -> List[TestCase]:
        ...
        function_context = self._extract_function_context(
            function_name, request.code_context
        )

        # Generate AI-powered tests if enabled
        if self.ai_enabled:
            try:
                ai_tests = await self._generate_ai_tests(
                    function_name, function_context, request
                )
                ...
                if self.fallback_to_templates:
                    template_tests = self._generate_template_tests(
...
"""
    ...
    def _parse_ai_test_response(
        self, response: str, function_name: str, request: TestGenerationRequest
    ) -> List[TestCase]:
    ...
    def _extract_tests_from_text(
        self, text: str, function_name: str, request: TestGenerationRequest
    ) -> List[TestCase]:
    ...
    async def _generate_class_tests(
        self, class_name: str, request: TestGenerationRequest
    ) -> List[TestCase]:
...
""",
    ...
    def _extract_function_context(
        self, function_name: str, code_context: CodeContext
    ) -> Dict[str, Any]:
    ...
    def _estimate_coverage(
        self, test_cases: List[TestCase], code_context: CodeContext
    ) -> float:
    ...
    def _calculate_quality_score(
        self, test_suite: TestSuite, coverage_analysis: Dict[CoverageType, float]
    ) -> float:
        ...
        if complex_tests:
            setup_ratio = sum(1 for tc in complex_tests if tc.setup_code) / len(
                complex_tests
            )
    ...
    def _identify_missing_coverage(self, test_suite: TestSuite) -> List[str]:
        ...
        error_tests = [
            ...
            for tc in test_suite.test_cases
            if any(
                "exception" in assertion.lower() or "error" in assertion.lower()
    ...
    def _generate_test_recommendations(
        ...
        if not any(
            tc.test_type == TestType.INTEGRATION for tc in test_suite.test_cases
        ):
        ...
        if not any(
            tc.test_type == TestType.PERFORMANCE for tc in test_suite.test_cases
        ):
            recommendations.append(
        ...
        if test_suite.test_cases:
            ...
            if avg_assertions < 2:
                recommendations.append(
                    "Increase assertion coverage - add more assertions per test"
                )
