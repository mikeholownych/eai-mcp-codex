...
async def generate_tests_from_file(file: UploadFile = File(...),
                                 test_types: str = "unit",
                                 framework: str = "pytest",
                                 coverage_target: float = 80.0) -> TestSuiteResponse:
...
async def generate_tests_for_function(function_name: str,
                                    function_code: str,
                                    language: str = "python",
                                    framework: str = "pytest",
                                    test_types: str = "unit") -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Function test generation failed: {str(e)}")
...
async def analyze_code_for_testing(code_content: str, filename: str) -> Dict[str, Any]:
    ...
    try:
        code_context = ai_test_generator.code_analyzer.analyze_code(code_content, filename)
        ...
        return {
            ...
            "analysis": {
                ...
                "testability_rating": "high" if code_context.complexity_score < 3 else 
                                   "medium" if code_context.complexity_score < 6 else "low"
            },
            "recommendations": [
            ...
            "estimated_test_count": len(code_context.functions) * 3 + len(code_context.classes) * 2,
            "status": "completed"
        }
...
async def validate_generated_tests(test_code: str, framework: str = "pytest") -> Dict[str, Any]:
    ...
    try:
        ...
        if "setUp" not in test_code and "setup" not in test_code:
            validation_results["warnings"].append("Consider adding setup code for complex tests")
        
        validation_results["overall_valid"] = (
            validation_results["syntax_valid"] and 
            validation_results["structure_valid"] and
            validation_results["test_count"] > 0
        )
