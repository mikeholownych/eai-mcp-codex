...
class CSRFProtection:
    ...
    def validate_token(self, session_id: str, provided_token: str) -> bool:
        ...
        if datetime.utcnow() - stored_data["created"] > timedelta(
            seconds=stored_data["ttl"]
        ):
...
class SecurityHeadersMiddleware:
    ...
    async def __call__(self, request: Request, call_next):
        ...
        try:
            ...
            if self.config.csrf_protection_enabled and request.method in [
                "POST",
                "PUT",
                "DELETE",
                "PATCH",
            ...
            return JSONResponse(
                status_code=500, content={"error": "Security middleware error"}
            )
    ...
    def _add_cors_headers(self, response: Response, request: Request):
        ...
        if origin and (
            ...
            or "*" in self.config.cors_allow_origins
        ):
...
class SecurityHeadersPresets:
    ...
    def development() -> SecurityHeadersConfig:
        ...
        return (
            ...
            .with_cors(allow_origins=["*"], allow_credentials=False)
            .build()
        )
