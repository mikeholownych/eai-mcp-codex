...
class EncryptionProvider(Protocol):
    ...
    async def encrypt(
        self, data: bytes, key: EncryptionKey, context: Optional[Dict[str, Any]] = None
    ) -> bytes: ...
...
class AESGCMProvider:
    ...
    async def encrypt(
        ...
        try:
            ...
            # Create cipher
            cipher = Cipher(algorithms.AES(key.key_material), modes.GCM(nonce))
    ...
    async def decrypt(
        ...
        try:
            ...
            # Create cipher
            cipher = Cipher(algorithms.AES(key.key_material), modes.GCM(nonce, tag))
...
class KeyManager:
    ...
    def __init__(
        self, config: EncryptionConfig, redis_client: Optional[RedisClient] = None
    ):
        self.config = config
    ...
    async def generate_key(
        self, key_type: KeyType, algorithm: Optional[EncryptionAlgorithm] = None
    ) -> EncryptionKey:
        ...
        if self.config.key_rotation_days > 0:
            key.expires_at = key.created_at + timedelta(
    ...
    async def rotate_key(self, key_id: str) -> EncryptionKey:
        ...
        new_key.metadata[f"generation_{old_key.rotation_count}"] = base64.b64encode(
    ...
    async def list_keys(
        self, key_type: Optional[KeyType] = None, include_inactive: bool = False
    ) -> List[EncryptionKey]:
    ...
    async def _load_master_key(self):
        ...
        if master_key_env:
            ...
            logger.warning(
                "Generated new master key. Store ENCRYPTION_MASTER_KEY environment variable."
            )
    ...
    async def _load_key(self, key_id: str) -> Optional[EncryptionKey]:
        ...
        try:
            if self.redis_client:
                key_data = await self.redis_client.client.get(
    ...
    async def _store_key(self, key: EncryptionKey):
        ...
        try:
            ...
            if self.redis_client:
                ...
                encrypted_data = self._encrypt_with_master_key(
                    json.dumps(key_dict).encode()
                )
                await self.redis_client.client.set(
                    f"encryption_key:{key.key_id}", encrypted_data
                )
                ...
                if key.expires_at:
                    ...
                    await self.redis_client.client.expire(
                        f"encryption_key:{key.key_id}", ttl
                    )
            else:
                # Store in memory
...
class EncryptionService:
    ...
    async def decrypt_field(
        self, encrypted_data: str, context: Optional[Dict[str, Any]] = None
    ) -> bytes:
    ...
    async def encrypt_payload(
        self, payload: Dict[str, Any], sensitive_fields: List[str]
    ) -> Dict[str, Any]:
    ...
    async def decrypt_payload(
        self, encrypted_payload: Dict[str, Any], sensitive_fields: List[str]
    ) -> Dict[str, Any]:
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            if encryption_service and isinstance(result, dict):
                result = await encryption_service.encrypt_payload(
