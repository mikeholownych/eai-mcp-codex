...
class InputSanitizer:
    ...
    def __init__(self, config: ValidationConfig):
        ...
        self.script_regex = [
            re.compile(pattern, re.IGNORECASE) for pattern in self.script_patterns
        ]
        ...
        if self.config.block_sql_keywords:
            for regex in self.sql_regex:
                if regex.search(value):
                    raise SecurityViolation(
                        "Potential SQL injection detected", "sql_injection", field_name
                    )
    ...
    def sanitize_dict(
        self, data: Dict[str, Any], path: str = "", depth: int = 0
    ) -> Dict[str, Any]:
    ...
    def sanitize_list(
        self, data: List[Any], path: str = "", depth: int = 0
    ) -> List[Any]:
        """Sanitize list items"""
...
class ValidationMiddleware:
    ...
    def add_validation_rule(
        self, path: str, method: str, validator_func: Callable[[Any], Any]
    ):
    ...
    async def _validate_request(self, request: Request):
        ...
        if request.query_params:
            self.sanitizer.sanitize_dict(dict(request.query_params))
    ...
    def _validate_headers(self, request: Request):
        ...
        dangerous_headers = ["x-forwarded-host", "x-original-host", "x-rewrite-url"]
    ...
    async def _validate_body(self, request: Request):
        ...
        if "application/json" in content_type:
            try:
                ...
                raise SecurityViolation(
                    "Invalid character encoding in request body", "encoding_error"
                )
