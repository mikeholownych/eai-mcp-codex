...
class AnomalousBehaviorDetector:
    ...
    async def detect(self, event_data: Dict[str, Any]) -> Optional[ThreatEvent]:
                ...
                anomalies.append(
                    {
                        ...
                        "evidence": {
                            ...
                            "typical_ips": list(profile.typical_ips)[:5],
                        },
                    }
                )
        ...
        if anomalies:
            ...
            event_id = (
                f"ab_{int(time.time())}_{hashlib.md5(user_id.encode()).hexdigest()[:8]}"
            )
    ...
    async def _update_user_profile(
        self, profile: UserBehaviorProfile, event_data: Dict[str, Any]
    ):
...
class SuspiciousIPDetector:
    ...
    async def detect(self, event_data: Dict[str, Any]) -> Optional[ThreatEvent]:
        ...
        if reputation_score >= 0.7:  # High risk threshold
            threat_level = (
                ThreatLevel.HIGH if reputation_score >= 0.9 else ThreatLevel.MEDIUM
            )
...
class MLAnomalyDetector:
    ...
    def _extract_features(self, event_data: Dict[str, Any]) -> Optional[List[float]]:
        ...
        try:
            ...
            features.append(
                float(event_data.get("event_type") == "security.rate_limit.exceeded")
            )
...
class ThreatDetectionEngine:
    ...
    async def resolve_threat(
        self, event_id: str, resolved_by: str, false_positive: bool = False
    ):
    ...
    def _map_threat_to_audit_severity(
