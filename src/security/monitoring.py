...
class SecurityMetricsCollector:
    ...
    async def record_metric(
        ...
        metric = SecurityMetric(
            name=name,
            metric_type=self.metric_definitions.get(name, {}).get(
                "type", MetricType.GAUGE
            ),
            value=value,
    ...
    async def _store_metric_in_redis(self, metric: SecurityMetric):
        ...
        try:
            ...
            await self.redis_client.client.set(
                key, json.dumps(metric.to_dict()), ex=self.retention_hours * 3600
            )
        except Exception as e:
            logger.error(f"Error storing metric in Redis: {e}")

    async def _load_metric_from_redis(
...
class SecurityAlerting:
    ...
    def __init__(
        self, metrics_collector: SecurityMetricsCollector, redis_client: RedisClient
    ):
    ...
    async def _evaluate_alert_rule(self, rule_id: str, rule: Dict[str, Any]):
        ...
        if "rate >" in condition:
            ...
            threshold = float(condition.split(">")[1].strip())
            current_value = (
                await self.metrics_collector.get_current_value(metric_name) or 0
            )
        elif "value <" in condition:
            threshold = float(condition.split("<")[1].strip())
            current_value = (
        ...
        if alert_triggered and alert_id not in self.active_alerts:
            ...
            logger.warning(
                f"Alert triggered: {alert.title} (value: {current_value}, threshold: {threshold})"
            )
...
class SecurityDashboard:
    ...
    async def _get_overview_metrics(self) -> Dict[str, Any]:
        ...
        return {
            "threats_last_hour": await self.metrics_collector.calculate_rate(
                "threats_detected_total", 60
            ),
            "incidents_open": len(
                [
                    i
                    for i in (
                        await self.incident_response.get_incidents()
            ...
            "active_sessions": await self.metrics_collector.get_current_value(
                "active_sessions"
            )
            or 0,
            "failed_logins_last_hour": await self.metrics_collector.calculate_rate(
                "failed_logins_total", 60
            ),
    ...
    async def _get_threat_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "detection_rate": await self.metrics_collector.calculate_rate(
                "threats_detected_total", 60
            ),
        }
    ...
    async def _get_incident_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "resolution_rate": stats.get("incidents_resolved", 0)
            / max(stats.get("incidents_created", 1), 1),
        }
    ...
    async def _generate_chart_data(self) -> Dict[str, Any]:
        ...
        threat_history = await self.metrics_collector.get_metric_history(
            "threats_detected_total", hours=24
        )
        if threat_history:
        ...
        login_history = await self.metrics_collector.get_metric_history(
            "failed_logins_total", hours=24
        )
        if login_history:
        ...
        session_history = await self.metrics_collector.get_metric_history(
            "active_sessions", hours=24
        )
        if session_history:
...
class SecurityMetricsMiddleware:
    ...
    async def __call__(self, request: Request, call_next):
        ...
        try:
            ...
            if request.url.path == "/auth/login" and response.status_code in [401, 403]:
                await self.metrics_collector.increment_counter(
                    "failed_logins_total",
                    labels={
                        "source_ip": (
...
async def setup_security_monitoring(
    ...
    incident_response: Optional[IncidentResponseEngine] = None,
    session_manager: Optional[SessionManager] = None,
) -> Tuple[SecurityMetricsCollector, SecurityAlerting, SecurityDashboard]:
...
def create_dashboard_routes(app: FastAPI, dashboard: SecurityDashboard):
    ...
    async def get_metric_data(metric_name: str, hours: int = 1):
        ...
        history = await dashboard.metrics_collector.get_metric_history(
            metric_name, hours
        )
        return {
            "metric_name": metric_name,
