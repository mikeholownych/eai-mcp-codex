...
class SessionData:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "device_fingerprint": {
                "user_agent": self.device_fingerprint.user_agent if self.device_fingerprint else None,
                "screen_resolution": self.device_fingerprint.screen_resolution if self.device_fingerprint else None,
                "timezone": self.device_fingerprint.timezone if self.device_fingerprint else None,
                "language": self.device_fingerprint.language if self.device_fingerprint else "en",
                "platform": self.device_fingerprint.platform if self.device_fingerprint else None,
                "browser_features": self.device_fingerprint.browser_features if self.device_fingerprint else {},
                "fingerprint_hash": self.device_fingerprint.fingerprint_hash if self.device_fingerprint else None
...
class SessionManager:
    ...
    async def create_session(self, 
                           ...
                           session_type: SessionType = SessionType.WEB,
                           security_level: SecurityLevel = SecurityLevel.MEDIUM,
                           expiry_minutes: Optional[int] = None) -> SessionData:
        """Create new session"""
        ...
        country, city = await self._get_location_from_ip(ip_address) if ip_address else (None, None)
        
        # Generate CSRF token
        csrf_token = secrets.token_urlsafe(32) if self.config.csrf_protection_enabled else None
    ...
    async def validate_session(self, 
                             ...
                             ip_address: Optional[str] = None,
                             user_agent: Optional[str] = None,
                             device_fingerprint: Optional[DeviceFingerprint] = None) -> Optional[SessionData]:
        ...
        if self.config.ip_validation_enabled and session_data.ip_locked:
            if ip_address != session_data.ip_address:
                await self._flag_anomaly(session_data, "ip_change", {
                    "original_ip": session_data.ip_address,
                    "new_ip": ip_address
                })
                
                # If too many IP changes, suspend session
                ip_changes = sum(1 for flag in session_data.anomaly_flags if flag.startswith("ip_change"))
                if ip_changes >= self.config.max_ip_changes:
                    await self._suspend_session(session_id, "Too many IP address changes")
                    return None
        
        # Validate device fingerprint
        if (self.config.device_fingerprinting_enabled and 
            session_data.device_locked and 
            session_data.device_fingerprint and 
            device_fingerprint):
            
            if not session_data.device_fingerprint.matches(device_fingerprint, self.config.device_fingerprint_tolerance):
                await self._flag_anomaly(session_data, "device_change", {
                ...
                if session_data.security_level in [SecurityLevel.HIGH, SecurityLevel.CRITICAL]:
                    await self._suspend_session(session_id, "Device fingerprint mismatch")
                    return None
    ...
    async def extend_session(self, session_id: str, additional_minutes: int) -> bool:
        ...
        if session_data.expires_at:
            ...
            session_data.expires_at = datetime.utcnow() + timedelta(minutes=additional_minutes)
    ...
    async def revoke_session(self, session_id: str, reason: str = "Manual revocation") -> bool:
        """Revoke session"""
        session_data = await self._get_session(session_id)
        if not session_data:
            return False
    ...
    async def revoke_user_sessions(self, user_id: str, exclude_session: Optional[str] = None, reason: str = "User logout") -> int:
    ...
    async def get_user_sessions(self, user_id: str, include_inactive: bool = False) -> List[SessionData]:
    ...
    async def _enforce_session_limits(self, user_id: str):
        ...
        if len(active_sessions) >= self.config.max_concurrent_sessions:
            ...
            await self.revoke_session(oldest_session_id, "Concurrent session limit exceeded")
    ...
    async def _update_session_activity(self, 
        ...
        if not last_activity or (current_time - datetime.fromisoformat(last_activity["timestamp"])).total_seconds() > 60:
            session_data.activity_log.append({
    ...
    async def _detect_anomalies(self, 
        ...
        if ip_address and self.config.geographic_validation_enabled:
            ...
            if (current_country and session_data.country and 
                current_country != session_data.country):
                anomalies.append("geographic_change")
        
        # User agent changes
        if (user_agent and session_data.user_agent and 
            ...
            if (current_ua.browser.family != original_ua.browser.family or
                current_ua.os.family != original_ua.os.family):
                anomalies.append("user_agent_change")
        
        # Rate-based anomalies
        if session_data.activity_log:
            recent_activities = [
                a for a in session_data.activity_log
                if (datetime.utcnow() - datetime.fromisoformat(a["timestamp"])).total_seconds() < 60
            ]
        ...
        for anomaly in anomalies:
            await self._flag_anomaly(session_data, anomaly, {
                "ip_address": ip_address,
                "user_agent": user_agent
            })
    
    async def _flag_anomaly(self, session_data: SessionData, anomaly_type: str, details: Dict[str, Any]):
        ...
        session_data.anomaly_flags = [
            flag for flag in session_data.anomaly_flags
        ...
        if session_data.risk_score >= self.config.anomaly_score_threshold:
            await self._suspend_session(session_data.session_id, f"High risk score: {session_data.risk_score}")
    ...
    async def _get_location_from_ip(self, ip_address: str) -> Tuple[Optional[str], Optional[str]]:
            ...
            for key in session_keys:
                ...
                if session_data_raw:
                    try:
                        ...
                        self.user_sessions[session_data.user_id].add(session_data.session_id)
                        ...
                        if session_data_raw:
                            ...
                            if expires_at:
                                ...
                                if datetime.utcnow() > expires_dt:
                                    ...
                                    await self._expire_session(session_id, "Cleanup - session expired")
                                    expired_count += 1
...
# Factory function
async def setup_session_manager(redis_client: RedisClient,
                               encryption_service: EncryptionService,
                               audit_logger: AuditLogger,
