        ...
        for key, value in updates.items():
            ...
            if not isinstance(value, expected_type) and value is not None:
                try:
                    ...
                    logger.error(f"Invalid type for {section}.{key}: expected {expected_type.__name__}")
                    continue
...
def initialize_security_config(config: Optional[SecurityConfig] = None) -> SecurityConfigManager:
