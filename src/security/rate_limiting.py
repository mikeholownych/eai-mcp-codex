...
class RateLimiter:
    ...
    async def check_rate_limit(
        ...
        if strategy == RateLimitStrategy.SLIDING_WINDOW:
            return await self._sliding_window_check(identifier, limit, window_seconds, current_time)
        elif strategy == RateLimitStrategy.TOKEN_BUCKET:
            return await self._token_bucket_check(identifier, limit, window_seconds, current_time)
        elif strategy == RateLimitStrategy.FIXED_WINDOW:
            return await self._fixed_window_check(identifier, limit, window_seconds, current_time)
        else:
    ...
    async def __call__(self, request: Request, call_next):
        ...
        for window_name, seconds in [("minute", 60), ("hour", 3600)]:
            ...
            if limit_key in self.rate_limiter.default_limits:
                ...
                allowed, info = await self.rate_limiter.check_rate_limit(
                    f"{identifier}:global:{window_name}",
                    limit,
                    seconds
                )
...
async def setup_rate_limiter(redis_client: RedisClient, settings: Settings) -> RateLimiter:
