...
def initiate_consensus(decision_topic: str, options: list, agents_involved: list):
    ...
    try:
        ...
        print(f"Consensus initiated. Collaboration ID: {result.get("collaboration_id")}")
        return result
    ...
    if consensus_init_result:
        ...
        if collaboration_id:
            ...
            for i in range(5):
                ...
                if status:
                    print(f"Current status ({i+1}/5): {status.get("status")}, Result: {status.get("result", "N/A")}")
                    if status.get("status") == "completed":
                        break
            print("\nConsensus process finished.")
            final_status = get_consensus_status(collaboration_id)
            if final_status:
                print(f"Final Consensus Result: {final_status.get("result", "No consensus reached")}")
        else:
