        ...
        for start in reversed(range(0, end, self._chunk_size)):
            ...
            with self._stay():
                try:
                    ...
                    compression_ratio = total_uncompressed / self._length if self._length > 0 else 0

                    # Reject suspicious compression ratios (e.g., > 100:1)
                    if compression_ratio > 100:
                        raise ValueError(
                            f"Suspicious compression ratio {compression_ratio:.1f}:1, possible ZIP bomb"
                        )
                except BadZipFile:
                    pass
                else:
        ...
        with self._stay():
            ...
            for start, end in self._merge(start, end, left, right):
                ...
                if downloaded_size != expected_size:
                    raise ValueError(f"Downloaded {downloaded_size} bytes but expected {expected_size}")
