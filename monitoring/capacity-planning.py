...
class MetricsCollector:
    ...
    async def query_prometheus(self, query: str, start_time: datetime, end_time: datetime) -> Dict:
    ...
    async def get_resource_metrics(self, lookback_days: int = 7) -> List[ResourceMetrics]:
        ...
        timestamps = set()
        if cpu_data:
            timestamps.update([float(point[0]) for point in cpu_data[0].get('values', [])])
    ...
    async def get_service_metrics(self, service: str, lookback_days: int = 7) -> List[ServiceMetrics]:
        ...
        timestamps = set()
        if rate_data:
            timestamps.update([float(point[0]) for point in rate_data[0].get('values', [])])
    ...
    def _extract_value_at_time(self, data: List[Dict], timestamp: float) -> Optional[float]:
...
class TrendAnalyzer:
    ...
    def analyze_resource_trends(self, metrics: List[ResourceMetrics]) -> Dict[str, CapacityPrediction]:
    ...
    def analyze_service_trends(self, service: str, metrics: List[ServiceMetrics]) -> Dict[str, CapacityPrediction]:
        ...
        thresholds = {
            ...
            'active_connections': self._calculate_dynamic_threshold(df['active_connections'])
        }
    ...
    def _predict_capacity(self, df: pd.DataFrame, metric: str, threshold: float, service: str = "system") -> CapacityPrediction:
        ...
        return CapacityPrediction(
            ...
            confidence_interval=best_prediction.get('confidence_interval', (current_value, current_value)),
            days_until_threshold=days_until_threshold,
            recommended_action=recommended_action,
            urgency=urgency
        )
    
    def _linear_regression(self, df: pd.DataFrame, metric: str, threshold: float) -> Dict:
        ...
        return {
            ...
            'confidence_interval': (predictions[-1] - 1.96 * std_error, predictions[-1] + 1.96 * std_error),
            'days_until_threshold': days_until_threshold,
            'score': mse
        }
    
    def _exponential_regression(self, df: pd.DataFrame, metric: str, threshold: float) -> Dict:
    ...
    def _seasonal_decomposition(self, df: pd.DataFrame, metric: str, threshold: float) -> Dict:
    ...
    def _simple_linear_prediction(self, df: pd.DataFrame, metric: str, threshold: float) -> Dict:
    ...
    def _determine_action(self, metric: str, current: float, threshold: float, days_until: Optional[int]) -> Tuple[str, str]:
...
class CapacityPlanner:
    ...
    def _load_config(self, config_path: Optional[str]) -> Dict:
        ...
        return {
            ...
            'services': ['model-router', 'plan-management', 'git-worktree-manager', 
                        'workflow-orchestrator', 'verification-feedback'],
            'lookback_days': 7,
    ...
    async def generate_capacity_report(self) -> Dict:
        ...
        async with MetricsCollector(self.config['prometheus_url']) as collector:
            ...
            resource_predictions = self.analyzer.analyze_resource_trends(resource_metrics)
            ...
            for service in self.config['services']:
                ...
                service_preds = self.analyzer.analyze_service_trends(service, service_metrics)
            ...
            report = {
                ...
                'alerts': self._generate_alerts(resource_predictions, service_predictions)
            }
            
            return report
    
    def _format_predictions(self, predictions: Dict[str, CapacityPrediction]) -> List[Dict]:
    ...
    def _generate_recommendations(self, resource_preds: Dict, service_preds: Dict) -> List[Dict]:
        ...
        for key, pred in sorted_preds[:10]:  # Top 10 recommendations
            recommendations.append({
                ...
                'timeline': f"{pred.days_until_threshold} days" if pred.days_until_threshold else "Monitor",
...
async def main():
    ...
    try:
        ...
        for item in report['resource_analysis']:
            print(f"• {item['metric'].upper()}: {item['current_value']}% "
        ...
        for item in report['service_analysis']:
            print(f"• {item['service']} {item['metric']}: {item['current_value']} "
                  f"(Predicted: {item['predicted_value']}) - {item['urgency'].upper()}")
